		...
		unsigned short port)
{
	...
	if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, (struct sockaddr*) &address, sizeof(address)) == -1)
	{
		ThrowErrnoException(env, "java/io/IOException", errno);
	}

	// Bind socket
	LogMessage(env, obj, "Binding to %s:%d", ip, port);
		...
		size_t bufferSize)
{
	...
	ssize_t recvSize = read(sd, buffer, bufferSize - 1);
		...
		size_t bufferSize)
{
	...
	ssize_t sentSize = write(sd, buffer, bufferSize);
		...
		unsigned short port)
{
	...
	LogMessage(env, obj, "Connecting to %s:%d", ip, port);
  ...
  (JNIEnv * env, jobject obj, jstring ip, jint port, jstring message)
{
	if (clientSocket > 0)
	{
		close(clientSocket);
		clientSocket = 0;
		LogMessage(env, obj, "Client stopped");
	}
	if (serverSocket > 0)
	{
		close(serverSocket);
		serverSocket = 0;
		LogMessage(env, obj, "Server stopped");
	}
}
